...
type Tab = "plan" | "pool" | "capture" | "overview" | "ledger" | "settle" | "activity" | "profile";
type Booking = {
  id: string;
  title: string;
  subtitle: string;
...
export default function HomeScreen() {
  ...
  const [splitProposed, setSplitProposed] = useState(false);
  const [capturedExpense, setCapturedExpense] = useState(false);
  ...
  function renderPlan() {
    ...
    return (
      <>
        ...
        <SectionHeader title="AI itinerary options" />
        <View style={styles.suggestionCard}><View style={styles.suggestionRow}><View style={[styles.suggestionIcon, { backgroundColor: colors.mint }]}><Icon name="savings" size={18} color={colors.mintText} /></View><View style={{ flex: 1 }}><View style={styles.suggestionTitleRow}><Text style={styles.suggestionTitle}>Cheaper: shared jeep instead of private cab</Text><View style={styles.sponsoredBadge}><Text style={styles.sponsoredText}>Sponsored</Text></View></View><Text style={styles.suggestionText}>Trade-off: 25 minutes longer · projected share decreases ₹620</Text></View><Icon name="chevron-right" size={18} color={colors.faint} /></View><View style={styles.suggestionRow}><View style={[styles.suggestionIcon, { backgroundColor: colors.lavender }]}><Icon name="upgrade" size={18} color={colors.lavenderText} /></View><View style={{ flex: 1 }}><Text style={styles.suggestionTitle}>Premium: upgrade to river-view stay</Text><Text style={styles.suggestionText}>Trade-off: better view and breakfast · projected share increases ₹1,450</Text></View><Icon name="chevron-right" size={18} color={colors.faint} /></View><Text style={styles.suggestionFooter}>Suggestions use dates, headcount, budget, opt-ins, and preferences. Organic ranking stays visible beside sponsored options.</Text></View>
  ...
  function renderCapture() {
    return (
      <>
        <View style={styles.captureHero}><View style={styles.captureHeroIcon}><Icon name="add-a-photo" size={24} color={colors.blue} /></View><Text style={styles.captureTitle}>Capture an expense</Text><Text style={styles.captureSubtitle}>Choose the path that matches how you paid. Every path assigns participants, category, and itinerary node.</Text></View>
        <View style={styles.capturePathCard}><View style={styles.capturePathIcon}><Icon name="travel-explore" size={20} color={colors.blue} /></View><View style={{ flex: 1 }}><Text style={styles.capturePathTitle}>Book in-app</Text><Text style={styles.capturePathText}>Search mock travel results, assign participants at booking time, and pay from the trip pool.</Text></View><Pressable onPress={() => Alert.alert("Travel search", "Mock results: The Fern Residency ₹24,000 · River View Cottages ₹19,800 · Dandeli Adventures ₹8,000. Select a result to create a participation-based booking.")} style={styles.captureAction}><Text style={styles.captureActionText}>Search</Text></Pressable></View>
        <View style={styles.capturePathCard}><View style={[styles.capturePathIcon, { backgroundColor: colors.mint }]}><Icon name="document-scanner" size={20} color={colors.mintText} /></View><View style={{ flex: 1 }}><Text style={styles.capturePathTitle}>Scan receipt</Text><Text style={styles.capturePathText}>OCR reads vendor, total, line items, date, category, and itinerary node. You review before confirming.</Text></View><Pressable onPress={() => { setCapturedExpense(true); Alert.alert("Receipt scanned", "AI extracted ₹3,600 Airport cabs, 13 Sep, Transport. Review the result before confirming."); }} style={[styles.captureAction, { backgroundColor: colors.mint }]}><Text style={[styles.captureActionText, { color: colors.mintText }]}>Scan</Text></Pressable></View>
        <View style={styles.capturePathCard}><View style={[styles.capturePathIcon, { backgroundColor: colors.peach }]}><Icon name="edit-note" size={20} color={colors.peachText} /></View><View style={{ flex: 1 }}><Text style={styles.capturePathTitle}>Manual entry</Text><Text style={styles.capturePathText}>Fast cash entry with amount, category, itinerary node, involved members, and who paid.</Text></View><Pressable onPress={() => setAddOpen(true)} style={[styles.captureAction, { backgroundColor: colors.peach }]}><Text style={[styles.captureActionText, { color: colors.peachText }]}>Enter</Text></Pressable></View>
        {capturedExpense ? <View style={styles.reviewCard}><Text style={styles.reviewEyebrow}>REVIEW BEFORE CONFIRMING</Text><Text style={styles.reviewTitle}>Airport cabs · ₹3,600</Text><Text style={styles.reviewText}>OCR line items: airport → hotel ₹2,000 · return transfer ₹1,600</Text><Text style={styles.reviewText}>Assigned node: Travel · paid by Amit · participants: Aisha, Rohan, Priya, Amit, Neha</Text><View style={styles.proposalActions}><Pressable onPress={() => { setCapturedExpense(false); Alert.alert("Receipt confirmed", "The corrected receipt was added to the ledger and reimbursable cash credit was recorded for Amit."); }} style={styles.confirmButton}><Text style={styles.confirmButtonText}>Confirm receipt</Text></Pressable><Pressable onPress={() => setCapturedExpense(false)} style={styles.cancelButton}><Text style={styles.cancelButtonText}>Correct</Text></Pressable></View></View> : null}
        <View style={styles.currencyCard}><View style={styles.aiCardHeader}><View style={[styles.aiIcon, { backgroundColor: colors.blueSoft }]}><Icon name="currency-exchange" size={18} color={colors.blue} /></View><View style={{ flex: 1 }}><Text style={styles.aiCardTitle}>Currency capture is always explicit</Text><Text style={styles.aiCardText}>Store original currency and amount, FX rate, spread, and foreign transaction fee — never a converted figure alone.</Text></View></View></View>
      </>
    );
  }
  ...
  function renderLedger() {
    return (
      <>
        ...
        <View style={styles.aiSplitCard}><View style={styles.aiCardHeader}><View style={styles.aiIcon}><Icon name="psychology" size={18} color={colors.lavenderText} /></View><View style={{ flex: 1 }}><Text style={styles.aiCardTitle}>Natural-language splitting</Text><Text style={styles.aiCardText}>Describe how to split this expense. AI proposes a split for your confirmation — it never commits automatically.</Text></View></View><TextInput value={splitInstruction} onChangeText={setSplitInstruction} placeholder="e.g. Dinner equally, wine only Aman and me" placeholderTextColor={colors.faint} style={styles.aiInput} multiline /><Pressable onPress={() => { if (!splitInstruction.trim()) { Alert.alert("Describe the split", "Tell us how this expense should be shared."); return; } setSplitProposed(true); }} style={({ pressed }) => [styles.aiPrimaryButton, pressed && styles.pressed]}><Icon name="auto-awesome" size={16} color="#FFFFFF" /><Text style={styles.aiPrimaryText}>Propose split</Text></Pressable>{splitProposed ? <View style={styles.proposalBox}><Text style={styles.proposalEyebrow}>AI PROPOSAL · REVIEW BEFORE COMMITTING</Text><Text style={styles.proposalText}>Dinner ₹4,800 equally across 4 people · wine ₹1,200 between Aman and Aisha</Text><Text style={styles.proposalAmounts}>Aisha ₹1,800 · Aman ₹1,800 · Rohan ₹1,200</Text><View style={styles.proposalActions}><Pressable onPress={() => { setSplitProposed(false); Alert.alert("Split confirmed", "The proposed split was committed as participation data; balances remain deterministic."); }} style={styles.confirmButton}><Text style={styles.confirmButtonText}>Confirm proposed split</Text></Pressable><Pressable onPress={() => setSplitProposed(false)} style={styles.cancelButton}><Text style={styles.cancelButtonText}>Edit</Text></Pressable></View></View> : null}</View>
        <View style={styles.currencyCard}><View style={styles.aiCardHeader}><View style={[styles.aiIcon, { backgroundColor: colors.blueSoft }]}><Icon name="currency-exchange" size={18} color={colors.blue} /></View><View style={{ flex: 1 }}><Text style={styles.aiCardTitle}>Multi-currency record</Text><Text style={styles.aiCardText}>Original amount is preserved alongside the FX rate and fees at the moment recorded.</Text></View></View><View style={styles.currencyRow}><Text style={styles.currencyLabel}>ORIGINAL</Text><Text style={styles.currencyValue}>$180.00 USD</Text><Text style={styles.currencyMeta}>FX 83.12 · recorded now</Text></View><View style={styles.currencyRow}><Text style={styles.currencyLabel}>TRIP BASE</Text><Text style={styles.currencyValue}>₹14,962.00</Text><Text style={styles.currencyMeta}>FX spread ₹120 · foreign fee ₹75</Text></View><Text style={styles.currencyFooter}>Aisha views balances in INR · settle within currency first, then across currencies.</Text></View>
  ...
  function renderActivity() {
    ...
    return (
      <>
        ...
        <View style={styles.inconsistencyCard}><View style={styles.inconsistencyHeader}><Icon name="warning-amber" size={19} color={colors.peachText} /><Text style={styles.inconsistencyTitle}>Booking inconsistency detection</Text></View><View style={styles.warningRow}><Icon name="error-outline" size={16} color={colors.peachText} /><Text style={styles.warningText}>Room booked for 2, but 3 people are assigned to its cost.</Text></View><View style={styles.warningRow}><Icon name="event-busy" size={16} color={colors.red} /><Text style={styles.warningText}>Waterfall detour has no opt-ins yet.</Text></View><View style={styles.warningRow}><Icon name="date-range" size={16} color={colors.peachText} /><Text style={styles.warningText}>Rafting and airport cab overlap on 13 Sep — review timing.</Text></View><Text style={styles.aiDisclaimer}>AI flags structural errors for review; deterministic ledger code remains the only source of balances.</Text></View>
    ...
    <ScreenContainer edges={["top", "left", "right", "bottom"]} containerClassName="bg-[#F5F8FC]">
      ...
      <View style={styles.appShell}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          {activeTab === "plan" ? renderPlan() : activeTab === "pool" ? renderPool() : activeTab === "capture" ? renderCapture() : activeTab === "overview" ? renderOverview() : activeTab === "ledger" ? renderLedger() : activeTab === "settle" ? renderSettle() : activeTab === "activity" ? renderActivity() : renderProfile()}
          <View style={{ height: 96 }} />
        </ScrollView>
        <View style={styles.bottomNav}>{(["plan", "pool", "capture", "overview", "ledger", "settle", "activity", "profile"] as Tab[]).map((tab) => { const meta = { plan: ["route", "Plan"], pool: ["account-balance", "Pool"], capture: ["add-circle-outline", "Capture"], overview: ["home", "Home"], ledger: ["account-balance-wallet", "Ledger"], settle: ["payments", "Settle"], activity: ["timeline", "Activity"], profile: ["person-outline", "Profile"] }[tab] as [keyof typeof MaterialIcons.glyphMap, string]; return <Pressable key={tab} onPress={() => setActiveTab(tab)} style={({ pressed }) => [styles.navItem, pressed && styles.pressed]}><Icon name={meta[0]} size={22} color={activeTab === tab ? colors.blue : colors.faint} /><Text style={[styles.navLabel, activeTab === tab && styles.navLabelActive]}>{meta[1]}</Text>{activeTab === tab ? <View style={styles.navIndicator} /> : null}</Pressable>; })}</View>
      </View>
...
const styles = StyleSheet.create({
  ...
  captureHero: { backgroundColor: colors.blueSoft, borderRadius: 18, padding: 18, marginBottom: 14 },
  captureHeroIcon: { width: 44, height: 44, borderRadius: 14, backgroundColor: colors.card, alignItems: "center", justifyContent: "center", marginBottom: 11 },
  captureTitle: { color: colors.ink, fontSize: 22, fontWeight: "800" },
  captureSubtitle: { color: colors.muted, fontSize: 11, lineHeight: 16, marginTop: 5 },
  capturePathCard: { backgroundColor: colors.card, borderRadius: 16, borderWidth: 1, borderColor: colors.line, padding: 13, flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 10 },
  capturePathIcon: { width: 40, height: 40, borderRadius: 12, backgroundColor: colors.blueSoft, alignItems: "center", justifyContent: "center" },
  capturePathTitle: { color: colors.ink, fontSize: 12, fontWeight: "800" },
  capturePathText: { color: colors.muted, fontSize: 9, lineHeight: 13, marginTop: 3 },
  captureAction: { backgroundColor: colors.blueSoft, borderRadius: 9, paddingHorizontal: 9, paddingVertical: 8 },
  captureActionText: { color: colors.blue, fontSize: 9, fontWeight: "800" },
  reviewCard: { backgroundColor: colors.mint, borderRadius: 15, padding: 13, marginTop: 4, marginBottom: 14 },
  reviewEyebrow: { color: colors.mintText, fontSize: 8, fontWeight: "800", letterSpacing: 0.8 },
  reviewTitle: { color: colors.ink, fontSize: 13, fontWeight: "800", marginTop: 6 },
  reviewText: { color: colors.muted, fontSize: 10, lineHeight: 14, marginTop: 5 },
  aiSplitCard: { backgroundColor: colors.lavender, borderRadius: 16, padding: 14, marginBottom: 12 },
  aiCardHeader: { flexDirection: "row", alignItems: "flex-start", gap: 9 },
  aiIcon: { width: 35, height: 35, borderRadius: 10, backgroundColor: "#E6DFFF", alignItems: "center", justifyContent: "center" },
  aiCardTitle: { color: colors.ink, fontSize: 12, fontWeight: "800" },
  aiCardText: { color: colors.muted, fontSize: 10, lineHeight: 14, marginTop: 3 },
  aiInput: { backgroundColor: colors.card, borderRadius: 10, paddingHorizontal: 10, paddingVertical: 9, minHeight: 46, color: colors.ink, fontSize: 10, marginTop: 11, textAlignVertical: "top" },
  aiPrimaryButton: { backgroundColor: colors.lavenderText, borderRadius: 10, height: 36, flexDirection: "row", gap: 6, alignItems: "center", justifyContent: "center", marginTop: 9 },
  aiPrimaryText: { color: "#FFFFFF", fontSize: 10, fontWeight: "800" },
  proposalBox: { backgroundColor: colors.card, borderRadius: 11, padding: 10, marginTop: 10 },
  proposalEyebrow: { color: colors.lavenderText, fontSize: 8, fontWeight: "800", letterSpacing: 0.7 },
  proposalText: { color: colors.ink, fontSize: 10, lineHeight: 14, marginTop: 6 },
  proposalAmounts: { color: colors.muted, fontSize: 9, marginTop: 5 },
  proposalActions: { flexDirection: "row", gap: 7, marginTop: 9 },
  confirmButton: { flex: 1, backgroundColor: colors.blue, borderRadius: 9, padding: 9, alignItems: "center" },
  confirmButtonText: { color: "#FFFFFF", fontSize: 9, fontWeight: "800" },
  cancelButton: { borderRadius: 9, borderWidth: 1, borderColor: colors.line, paddingHorizontal: 13, paddingVertical: 9, alignItems: "center" },
  cancelButtonText: { color: colors.muted, fontSize: 9, fontWeight: "800" },
  currencyCard: { backgroundColor: colors.card, borderRadius: 16, borderWidth: 1, borderColor: colors.line, padding: 13, marginBottom: 16 },
  currencyRow: { borderTopWidth: 1, borderTopColor: colors.line, paddingTop: 9, marginTop: 9 },
  currencyLabel: { color: colors.faint, fontSize: 8, fontWeight: "800", letterSpacing: 0.7 },
  currencyValue: { color: colors.ink, fontSize: 13, fontWeight: "800", marginTop: 4 },
  currencyMeta: { color: colors.muted, fontSize: 9, marginTop: 3 },
  currencyFooter: { color: colors.blue, fontSize: 9, lineHeight: 13, marginTop: 10 },
  suggestionCard: { backgroundColor: colors.card, borderRadius: 16, borderWidth: 1, borderColor: colors.line, paddingHorizontal: 13, marginBottom: 18 },
  suggestionRow: { flexDirection: "row", alignItems: "center", gap: 9, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: colors.line },
  suggestionIcon: { width: 35, height: 35, borderRadius: 10, alignItems: "center", justifyContent: "center" },
  suggestionTitleRow: { flexDirection: "row", alignItems: "center", gap: 5 },
  suggestionTitle: { color: colors.ink, fontSize: 10, fontWeight: "800", flexShrink: 1 },
  suggestionText: { color: colors.muted, fontSize: 9, lineHeight: 13, marginTop: 3 },
  sponsoredBadge: { backgroundColor: colors.peach, borderRadius: 5, paddingHorizontal: 4, paddingVertical: 3 },
  sponsoredText: { color: colors.peachText, fontSize: 7, fontWeight: "800" },
  suggestionFooter: { color: colors.faint, fontSize: 9, lineHeight: 13, paddingVertical: 10 },
  inconsistencyCard: { backgroundColor: colors.peach, borderRadius: 16, padding: 13, marginTop: 18, marginBottom: 14 },
  inconsistencyHeader: { flexDirection: "row", alignItems: "center", gap: 7 },
  inconsistencyTitle: { color: colors.ink, fontSize: 12, fontWeight: "800" },
  warningRow: { flexDirection: "row", alignItems: "flex-start", gap: 7, marginTop: 10 },
  warningText: { flex: 1, color: colors.muted, fontSize: 10, lineHeight: 14 },
  aiDisclaimer: { color: colors.peachText, fontSize: 9, lineHeight: 13, marginTop: 10 },
